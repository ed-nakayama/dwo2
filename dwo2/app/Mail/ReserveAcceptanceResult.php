 ...
 ***************************************************************/
class ReserveAcceptanceResult extends Mailable
{
     ...
     * @return void
     */
    public function __construct($title, $mailTo, $agentView, $weborderdetailList, $weborderheader, $acceptance)
    {
        $this->title              = $title;
        $this->mailTo             = $mailTo;
    ...
    public function build()
    {
		...
		return $this->to($this->mailTo)
			...
			->from($from)
			->subject($this->title)
			->text('mail_templates.reserve_acceptanceresult') // 本文
			->with([
